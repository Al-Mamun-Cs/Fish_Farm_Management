...
namespace SchoolManagement.Application.Features.Modules.Handlers.Queries
{
    ...
    { 
        ...
        public GetModuleListRequestHandler(ISchoolManagementRepository<Module> ModuleRepository, IMapper mapper, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<PagedResult<ModuleDto>> Handle(GetModuleListRequest request, CancellationToken cancellationToken)
        {
            ...
            var permission = _ModuleRepository.GetPermitedRoleFeatures(DeclareFeatureCode.MODULE, _httpContextAccessor.HttpContext.User.FindFirst(CustomClaimTypes.Rid)?.Value);
            var ModulesDtos = _mapper.Map<List<ModuleDto>>(Modules); 
            var result = new PagedResult<ModuleDto>(ModulesDtos, totalCount, request.QueryParams.PageNumber, request.QueryParams.PageSize, permission);
