...
{
    public class GetSupplierListByStatusRequestHandler : IRequestHandler<GetSupplierListByStatusRequest, PagedResult<SupplierDto>>
    {
        ...
        public GetSupplierListByStatusRequestHandler(ISchoolManagementRepository<Supplier> SupplierRepository, IMapper mapper, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<PagedResult<SupplierDto>> Handle(GetSupplierListByStatusRequest request, CancellationToken cancellationToken)
        {
            ...
            IQueryable<Supplier> Suppliers = _SupplierRepository.FilterWithInclude(x => (request.WarehouseId == 0 || x.WarehouseId == request.WarehouseId) && (request.SupplierStatus == 0 || x.SupplierStatus == request.SupplierStatus) && (x.SupplierName.Contains(request.QueryParams.SearchText) || String.IsNullOrEmpty(request.QueryParams.SearchText)), "Warehouse", "SupplierType", "Division", "District", "Upozila");
