...
@Component({
  selector: 'app-shopgoodsalevoucher-list',
  templateUrl: './shopgoodsalevoucher-list.component.html',
  styleUrls: ['./shopgoodsalevoucher-list.component.sass']
})
export class ShopGoodSaleVoucherListComponent implements OnInit {
  ...
  warehouse: any;
  transportCost: any;
  grandTotalSalePrice: any;
  saleLessAmount: any;
  customerPaidAmount: any;
  customerDueAmount: any;
  statusName: any;
  bankName: any;
  ...
  constructor(private snackBar: MatSnackBar, private authService: AuthService, private fb: FormBuilder, private ShopGoodSaleService: ShopGoodSaleService, private router: Router, private confirmService: ConfirmService, private route: ActivatedRoute, private _location: Location,) { }
  ...
  ngOnInit() {
    ...
    this.SpGetShopGoodSaleVoucherById(id);
  }
  backClicked() {
    this._location.back();
  }

  SpGetShopGoodSaleVoucherById(shopGoodSaleId: any) {
    this.ShopGoodSaleService.SpGetShopGoodSaleVoucherById(shopGoodSaleId).subscribe(response => {
      ...
      this.saleDate = response[0].saleDate;
      this.warehouse = response[0].warehouseName;
      this.transportCost = response[0].transportCost;
      this.grandTotalSalePrice = response[0].grandTotalSalePrice;
      this.saleLessAmount = response[0].saleLessAmount;
      this.customerPaidAmount = response[0].customerPaidAmount;
      this.customerDueAmount = response[0].customerDueAmount;
      ...
      this.printVoucher(shopGoodSaleId);
      ...
      <html>
        ...
        <body onload="window.print();window.close()">
          ...
          <img src="assets/images/ITH_Logo.png" alt="BMU Logo" style="height: 60px; margin-bottom: 5px; display: block; margin-left: auto; margin-right: auto;" />
            <h4 style="margin:0;"> পণ্য বিক্রয় রশিদ</h4>
          </div>
          <br>
          <hr>
  ...
  onSubmit() {
    var shopGoodSaleId = this.MosariSaleForm.value['shopGoodSaleId'];
