...
@Component({
  selector: 'app-fisheriesinventory-list',
  templateUrl: './fisheriesinventory-list.component.html',
  styleUrls: ['./fisheriesinventory-list.component.sass']
})
export class FisheriesInventoryListComponent implements OnInit {
  ...
  constructor(private snackBar: MatSnackBar,private authService: AuthService,private FisheriesInventoryService:FisheriesInventoryService,private router: Router,private confirmService: ConfirmService) { }
  ...
  ngOnInit() {
    ...
    this.getFisheriesInventorys();
  }
  
  getFisheriesInventorys() {
    this.isLoading = true;
    this.FisheriesInventoryService.getFisheriesInventorys(this.paging.pageIndex, this.paging.pageSize,this.searchText,0, this.branchId).subscribe(response => {
  ...
  deleteItem(row) {
    ...
    this.confirmService.confirm('Confirm delete message', 'Are You Sure Delete This  Item?').subscribe(result => {
      console.log(result);
      if (result) { 
        this.FisheriesInventoryService.delete(id).subscribe(() => {
          this.getFisheriesInventorys();
