...
@Component({
  selector: 'app-supplierdueamount-list',
  templateUrl: './supplierdueamount-list.component.html',
  styleUrls: ['./supplierdueamount-list.component.sass']
})
export class SupplierDueAmountListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getTotalDueAmountList();
  }
  backClicked() {
    this._location.back();
  }
  getTotalDueAmountList() {
    this.isLoading = true;
    this.DashboardService.getTotalDueAmountList(this.branchId,).subscribe(response => {
      this.dueAmount = response;
      console.log(this.dueAmount, "product Stock data")
  ...
  get grandTotalAmount(): number {
    return this.dueAmount?.reduce((sum, item) => {
      ...
      <html>
        ...
        <body onload="window.print();window.close()">
          ...
          <img src="assets/images/ITH_Logo.png" alt="BMU Logo" style="height: 60px; margin-bottom: 5px; display: block; margin-left: auto; margin-right: auto;" />
            <h4 style="margin:0;"> সরবরাহকারীর বকেয়া তালিকা</h4>
          </div>
          <br>
          <hr>
