...
@Component({
  selector: 'app-investordetailcapitalsix-list',
  templateUrl: './investordetailcapitalsix-list.component.html',
  styleUrls: ['./investordetailcapitalsix-list.component.sass']
})
export class InvestorDetailCapitalSixListComponent implements OnInit {
  ...
  grandTotalinvestmenAmount = 0;
  grandTotalprincipalReturn = 0;
  grandTotaldueInvestmenAmount = 0;
  grandTotalprofit= 0;
  searchText = "";
  permission: any;
  ...
  ngOnInit() {
    ...
    this.SpGetDepstInvestmentDetail();
  }
  backClicked() {
    this._location.back();
  }
  SpGetDepstInvestmentDetail() {
    this.isLoading = true;
    this.DashboardService.SpGetDepstInvestmentDetail(this.branchId,).subscribe(response => {
      this.investorDetail = response;
      this.getGrandTotal();
  ...
  getGrandTotal() {
    this.grandTotalinvestmenAmount = this.investorDetail.reduce(
      (sum, item) => sum + (item.investmenAmount || 0), 0);

    this.grandTotalprincipalReturn = this.investorDetail.reduce(
      (sum, item) => sum + (item.principalReturn || 0), 0);

    this.grandTotaldueInvestmenAmount = this.investorDetail.reduce(
      (sum, item) => sum + (item.dueInvestmenAmount || 0), 0);

    this.grandTotalprofit = this.investorDetail.reduce(
      ...
      <html>
        ...
        <body onload="window.print();window.close()">
          ...
          <img src="assets/images/ITH_Logo.png" alt="BMU Logo" style="height: 60px; margin-bottom: 5px; display: block; margin-left: auto; margin-right: auto;" />
            <h4 style="margin:0;">বিনিয়োগের  বিবরণ</h4>
          </div>
          <br>
          <hr>
