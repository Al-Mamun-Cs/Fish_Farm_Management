...
@Component({
  selector: 'app-depositamountdetail-list',
  templateUrl: './depositamountdetail-list.component.html',
  styleUrls: ['./depositamountdetail-list.component.sass']
})
export class DepositAmountDetailListComponent implements OnInit {
  ...
  grandTotalAmount = 0;
  grandTotalPresentBalance = 0;
  grandTotalDueAmount = 0;
  searchText = "";
  permission: any;
  monthlyInstallment: string = '0';
  startDate: string = '0';
  ...
  ngOnInit() {
    ...
    this.SpGetDepositAmountDetail();
  }
  backClicked() {
    this._location.back();
  }
  SpGetDepositAmountDetail() {
    this.isLoading = true;
    this.DashboardService.SpGetDepositAmountDetail(this.branchId,).subscribe(response => {
      this.depositAmountDetail = response;
      this.getGrandTotal();
      console.log(this.depositAmountDetail, "product Stock data")
      this.isLoading = false;

      this.startDate = String(response[0]?.startDate ?? 0);
      this.monthlyInstallment = String(response[0]?.monthlyInstallment ?? 0);
      console.log(this.monthlyInstallment, this.startDate, "monthlyInstallment")
  ...
  getGrandTotal() {
    this.grandTotalAmount = this.depositAmountDetail.reduce(
      (sum, item) => sum + (item.totalAmount || 0), 0);

    this.grandTotalPresentBalance = this.depositAmountDetail.reduce(
      (sum, item) => sum + (item.presentBalance || 0), 0);

    this.grandTotalDueAmount = this.depositAmountDetail.reduce(
      (sum, item) => sum + (item.dueAmount || 0), 0);
      ...
      <html>
        ...
        <body onload="window.print();window.close()">
          ...
          <img src="assets/images/ITH_Logo.png" alt="BMU Logo" style="height: 60px; margin-bottom: 5px; display: block; margin-left: auto; margin-right: auto;" />
            <h4 style="margin:0;">কিস্তির বিবরণ</h4>
          </div>
          <br>
          <hr>
