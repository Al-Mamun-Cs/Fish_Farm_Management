...
@Component({
  selector: 'app-depositorinstallmentdetail-list',
  templateUrl: './depositorinstallmentdetail-list.component.html',
  styleUrls: ['./depositorinstallmentdetail-list.component.sass']
})
export class DepositorInstallmentDetailListComponent implements OnInit {
  ...
  supplierId: any;
  depositorId: any;
  depositorInstallmentDetail: any;
  ...
  ngOnInit() {
    ...
    this.depositorId = this.route.snapshot.paramMap.get('depositorId');
    
    this.SpGetDepositorInstallmentDetail();
  }
  backClicked() {
    this._location.back();
  }
  SpGetDepositorInstallmentDetail() {
    this.isLoading = true;
    this.DashboardService.SpGetDepositorInstallmentDetail(this.depositorId,).subscribe(response => {
      this.depositorInstallmentDetail = response;
      this.getGrandTotal();
      console.log(this.depositorInstallmentDetail, "product Stock data")
  ...
  getGrandTotal() {
    this.grandInstallmentAmount = this.depositorInstallmentDetail.reduce(
      (sum, item) => sum + (item.installmentAmount || 0), 0);
