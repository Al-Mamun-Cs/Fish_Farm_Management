...
@Component({
  selector: 'app-dailysaleamount-list',
  templateUrl: './dailysaleamount-list.component.html',
  styleUrls: ['./dailysaleamount-list.component.sass']
})
export class DailySaleAmountListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getDailySaleAmountList();
  }
  backClicked() {
    this._location.back();
  }
  getDailySaleAmountList() {
    this.isLoading = true;
    this.DashboardService.getDailySaleAmountList(this.branchId,).subscribe(response => {
      this.saleAmount = response;
      console.log(this.saleAmount, "Sale Amount data")
  ...
  get grandTotalAmount(): number {
    return this.saleAmount?.reduce((sum, item) => {
      return sum + Number(item.totalSalePrice || 0);
    }, 0) || 0;
  }
      ...
      <html>
        ...
        <body onload="window.print();window.close()">
          ...
          <img src="assets/images/ITH_Logo.png" alt="BMU Logo" style="height: 60px; margin-bottom: 5px; display: block; margin-left: auto; margin-right: auto;" />
            <h4 style="margin:0;"> দৈনিক বিক্রয় তালিকা</h4>
          </div>
          <br>
          <hr>
