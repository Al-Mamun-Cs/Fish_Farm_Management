...
@Component({
  selector: 'app-customerdueamount-list',
  templateUrl: './customerdueamount-list.component.html',
  styleUrls: ['./customerdueamount-list.component.sass']
})
export class CustomerDueAmountListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getTotalCustomerDueAmountList();
  }
  backClicked() {
    this._location.back();
  }
  getTotalCustomerDueAmountList() {
    this.isLoading = true;
    this.DashboardService.getTotalCustomerDueAmountList(this.branchId,).subscribe(response => {
      this.customerDueAmount = response;
      console.log(this.customerDueAmount, "product Stock data")
  ...
  get grandTotalAmount(): number {
    return this.customerDueAmount?.reduce((sum, item) => {
      return sum + Number(item.totalDueAmount || 0);
    }, 0) || 0;
  }
      ...
      <html>
        ...
        <body onload="window.print();window.close()">
          ...
          <img src="assets/images/ITH_Logo.png" alt="BMU Logo" style="height: 60px; margin-bottom: 5px; display: block; margin-left: auto; margin-right: auto;" />
            <h4 style="margin:0;"> কাস্টমারের বকেয়া তালিকা</h4>
          </div>
          <br>
          <hr>
