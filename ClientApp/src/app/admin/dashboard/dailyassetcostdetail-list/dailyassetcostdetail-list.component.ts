...
@Component({
  selector: 'app-dailyassetcostdetail-list',
  templateUrl: './dailyassetcostdetail-list.component.html',
  styleUrls: ['./dailyassetcostdetail-list.component.sass']
})
export class DailyAssetCostDetailListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getDailyCostDetailList();
  }
  backClicked() {
    this._location.back();
  }
  getDailyCostDetailList() {
    this.isLoading = true;
    this.DashboardService.getDailyAssetCostDetailList(this.branchId,).subscribe(response => {
      this.dailyCostDetail = response;
      console.log(this.dailyCostDetail, "product Stock data")
  ...
  get grandTotalAmount(): number {
    return this.dailyCostDetail?.reduce((sum, item) => {
      ...
      <html>
        ...
        <body onload="window.print();window.close()">
          ...
          <img src="assets/images/ITH_Logo.png" alt="BMU Logo" style="height: 60px; margin-bottom: 5px; display: block; margin-left: auto; margin-right: auto;" />
            <h4 style="margin:0;"> দৈনিক খরচের তালিকা</h4>
          </div>
          <br>
          <hr>
