...
@Component({
  selector: 'app-investordetail-list',
  templateUrl: './investordetail-list.component.html',
  styleUrls: ['./investordetail-list.component.sass']
})
export class InvestorDetailListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.SpGetInvestorDetailList();
  }
  backClicked() {
    this._location.back();
  }
  SpGetInvestorDetailList() {
    this.isLoading = true;
    this.DashboardService.SpGetInvestorDetailList(this.branchId,).subscribe(response => {
  ...
  getGrandTotal() {
    this.grandTotalCash = this.investorDetail.reduce(
      (sum, item) => sum + (item.investAmount || 0), 0);

    this.grandTotalBank = this.investorDetail.reduce(
      (sum, item) => sum + (item.returnInvestAmount || 0), 0);

    this.grandTotalCapital = this.investorDetail.reduce(
      (sum, item) => sum + (item.profitAmount || 0), 0);

    this.grandTotalReturnableAmount = this.investorDetail.reduce(
      (sum, item) => sum + ((item.investAmount || 0) - (item.returnInvestAmount || 0)), 0);
  }
