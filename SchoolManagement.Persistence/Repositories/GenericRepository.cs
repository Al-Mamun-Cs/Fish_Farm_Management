...
namespace SchoolManagement.Persistence.Repositories
{
    public class GenericRepository<TDbContext,T> : IGenericRepository<T> where T : class where TDbContext : DbContext
    {
        ...
        public async Task<T> GetWithIncludes(Expression<Func<T, bool>> predicate, params Expression<Func<T, object>>[] includes)
        {
        ...
        public async Task<int> ExecuteSqlRawAsync(string sqlCommand)
        {
