...
public class RoleFeatureController : ControllerBase
{
    ...
    [HttpGet]
    [Route("get-RoleFeaturesByRole")]
    public async Task<ActionResult<List<RoleFeatureDto>>> GetRoleFeaturesByRole(string roleId)
    {
        var RoleFeatures = await _mediator.Send(new GetRoleFeaturesByRoleRequest { roleId = roleId });
        return Ok(RoleFeatures);
    }
    [HttpGet]
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesDefaultResponseType]
    [Route("assign-RoleFeature")]
    public async Task<ActionResult> Put(string roleId, RoleFeatureDto rfeature)
    {
        var command = new AssignRoleFeatureCommand
        {
            RoleFeatureDto = rfeature,
            RoleId = roleId
        };
        await _mediator.Send(command);
        return NoContent();
    ...
    //[HttpPut]
    //[Route("assign-RoleFeature")]
    //public async Task<ActionResult> AssignRoleFeatures(string roleId, RoleFeatureDto rfeature)
    //{
    //    var command = new AssignRoleFeatureCommand
    //    {
    //        RoleFeatureDto = rfeature,
